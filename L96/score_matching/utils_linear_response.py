...
'''
Authors: Fabri Falasca. Contact: fabrifalasca@gmail.com
...
# Function to compute the covariance matrix C(0)

--- RESPONSE C_0 REGULARIZED 

# Compute the response operator R(t) by pass C as an argument
This useful when we want to use a regularized C

--- TIKHONOV REGULARIZATION

# Regularize a the covariance matrix C
...
def analytical_bounds(x_t,phi,sigmas,tau,s,standardized='yes'):
...
########## Function to compute the covariance matrix C_0

def compute_C_0(x_t,standardized):
...
########## Response function where C_0 is given as an argument

def response_C_0_regularized(x_t,tau_max,C_0_regularized,standardized):
    ...
    # C_0_computed: the covariance matrix has been already computed and given as an argument.
    #               Note: in this case be careful that this C_0 was computed with the same option 
    #                     "standardized" used below.
    ...
    ########## Compute inverse of the covariance matrix
    inverse_C0 = inv(C_0_regularized)
    ...
    for tau in range(tau_max):
        ...
        if tau == 0:
            ########## Compute the response matrix
            response_matrix.append(np.matmul(C_0_regularized,inverse_C0))
        elif tau>0:    
            orbit1 = x_t[:,tau:]  # remove the FIRST tau elements
            orbit2 = x_t[:,:n_time-tau]  # remove the LAST tau elements
            # Compute the covariance matrix at lag tau
...
# Matrix C is regularized as C + \lambda I

def tickhonov_reg(x_t,a,standardized):
    ...
    # - a: "strength" of the regularization. 
    # The regularization parameter lambda is defined as a*\lambda_{max}.
    # Where \lambda_{max} is the max eigenvalue of the covariance matrix C
    ...
    # (correlation matrix if standardized = 'yes')
    C_0 = compute_C_0(x_t,standardized)
    
    print('Condition number of the C matrix is: '+str(np.linalg.cond(C_0)))
    
    # Tikhonov regularization
    max_lambda = np.linalg.eig(C_0)[0].max()
    lambda_reg = a*max_lambda
    Tikhonov = np.eye(C_0.shape[0])*lambda_reg
    # Tikhonov regularized
    C_0_reg = C_0 + Tikhonov
    
    print('Condition number of the regularized C matrix is: '+str(np.linalg.cond(C_0_reg)))
