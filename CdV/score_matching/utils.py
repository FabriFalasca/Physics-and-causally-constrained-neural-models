...
# Function to compute the linear response of a field given a specified forcing.
# The result lives in the low dimensional EOF space.
'''
...
# Function for linear response

from scipy import integrate

def linear_response(x_t, tau_max, standardized, significance, dx, forcing):
    
    ### Inputs
    # x_t: orbit. np.shape(x_t) = dimensions, T (length of time series)
    # tau_max: compute responsed up to tau
    # standardized: if 'yes' we standardize the time series to unit variance
    # significance: if 'yes' we use use the 3 sigma confidence bounds
    # dx: dx for the integration using trapezoidal rule
    # forcing: forcing pattern
    
    ### Outputs
    # linear response: linear repsonse to the forcing pattern
    
    # Compute response
    response_matrix = response(x_t,tau_max,standardized)
    
    # Confidence bounds?
    if significance == 'yes':
        
        # We need to compute the confidence bounds
        ...
        s_minus, s_plus = compute_quantile_analytical_tau_discrete(x_t,phi,sigmas,tau_max,s,standardized)
                
        # if you are not significant we change you to zero (we are sorry)
        indices = (response_matrix < s_plus) & (response_matrix > s_minus)
        response_matrix[indices] = 0
        
    # Compute theta operator
    # Integration donw using the trapezoidal rule. dx = 1 in this case
    theta_operator = integrate.trapezoid(response_matrix,dx=dx,axis=0)
    
    # Compute response
    average_response = np.dot(theta_operator,forcing)
